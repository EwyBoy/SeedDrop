...
public class SeedLoader {
    ...
    public static void registerSeeds() throws NumberFormatException {
        ...
        for (int i = 0; i < ConfigLoader.customDrops.length; i += 2) {
            ...
            if (splitStringItem.length < 2 || splitStringItem.length > 3) {
                SeedDrop.LOGGER.error("Invalid item string syntax: '" + configStringItem + "', should be '<domain>:<id>', or '<domain>:<id>:<meta>'");
                continue;
            }
            int meta = 0;
